...
function isMainGroupFolder(folder: string): boolean {
  return Object.values(registeredGroups).some(
    (group) => group.isMain === true && group.folder === folder,
  );
}
...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  let batchCursorStart = lastAgentTimestamp[chatJid] || '';
  const batches = splitMessagesByReplyTarget(missedMessages);
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      if (output === 'error' || hadError) {
        ...
        if (outputSentToUser) {
          logger.warn(
            { group: group.name },
            'Agent error after output was sent, keeping current batch cursor to prevent duplicates',
          );
          batchCursorStart = lastAgentTimestamp[chatJid];
          continue;
        }

        // Roll back cursor so retries can re-process the failed batch.
        lastAgentTimestamp[chatJid] = batchCursorStart;
        saveState();
        logger.warn(
          { group: group.name },
...
async function runAgent(
  ...
  const sessionNamespace = getSessionNamespace(
    group.folder,
    options.sendOptions?.threadTs,
  );
  const sessionId = sessions[sessionNamespace];
  ...
  const wrappedOnOutput = options.onOutput
    ? async (output: ContainerOutput) => {
        if (output.newSessionId) {
          sessions[sessionNamespace] = output.newSessionId;
          setSession(sessionNamespace, output.newSessionId);
        }
  ...
  try {
    const output = await runContainerAgent(
      group,
      {
        ...
        chatJid,
        replyThreadTs: options.sendOptions?.threadTs,
        isMain,
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          const batches = splitMessagesByReplyTarget(messagesToSend);
          ...
          if (batches.length === 1) {
            ...
            if (queue.sendMessage(chatJid, formatted, batch.sendOptions)) {
              logger.debug(
                { chatJid, count: batch.messages.length },
            ...
            queue.enqueueMessageCheck(chatJid, batch.sendOptions);
            continue;
          }

          queue.enqueueMessageCheck(chatJid, undefined, true);
        }
      }
    } catch (err) {
...
async function main(): Promise<void> {
  ...
  const channelNames = getRegisteredChannelNames().filter(
    (channelName) => !SLACK_ONLY || channelName === 'slack',
  );
  ...
  startIpcWatcher({
    ...
    sendReaction: (jid, emoji, options) => {
      const channel = findChannel(channels, jid);
      if (!channel?.sendReaction) {
        throw new Error(`Channel does not support reactions for JID: ${jid}`);
      }
      return channel.sendReaction(jid, emoji, options);
    },
    sendFile: (jid, filePath, options) => {
      const channel = findChannel(channels, jid);
      if (!channel?.sendFile) {
        throw new Error(`Channel does not support file upload for JID: ${jid}`);
      }
    ...
    writeGroupsSnapshot: (gf, im, ag, rj) =>
      writeGroupsSnapshot(gf, im, ag, rj),
    askAtlas: (sourceGroup, instruction, context) => {
      if (!isMainGroupFolder(sourceGroup)) {
        throw new Error('Atlas can only be called from the main group.');
      }
      return askAtlas(instruction, context);
    },
  });
