...
describe('formatMessages', () => {
  ...
  it('formats a single message as XML with context header', () => {
    ...
    expect(result).toContain('<message id="1" scope="main" sender="Alice"');
  ...
  it('includes thread_ts when present', () => {
    ...
    expect(result).toContain('scope="thread"');
  });

  it('includes message ids for tool references', () => {
    const result = formatMessages([makeMsg({ id: '1704067200.000000' })], TZ);
    expect(result).toContain('id="1704067200.000000"');
  });
});
