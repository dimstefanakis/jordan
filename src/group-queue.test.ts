...
describe('GroupQueue', () => {
  ...
  it('preempts an active container when a new message targets a different thread', async () => {
    const fs = await import('fs');
    let resolveProcess: () => void;
    ...
    queue.enqueueMessageCheck('group1@g.us', { threadTs: 'thread-b' });
  ...
  it('preempts an active container when batching requires a different run', async () => {
    const fs = await import('fs');
    let resolveProcess: () => void;
    ...
    queue.enqueueMessageCheck('group1@g.us', undefined, true);
  ...
  it('preempts idle container when a new message arrives', async () => {
    const fs = await import('fs');
    let resolveProcess: () => void;
  ...
  it('sendMessage returns false when the active reply target differs', async () => {
    let resolveProcess: () => void;
  ...
  it('preempts immediately when pending messages target a different reply context', async () => {
    const fs = await import('fs');
    let resolveProcess: () => void;
