...
describe('ask_atlas authorization', () => {
  it('main group can call Atlas', async () => {
    ...
    await processTaskIpc(
      {
        type: 'ask_atlas',
        requestId,
        instruction: 'Update the referrals docs',
        context: 'A user report showed the flow changed.',
      },
      'whatsapp_main',
      true,
      deps,
    ...
    expect(askAtlasSpy).toHaveBeenCalledWith(
      'whatsapp_main',
      'Update the referrals docs',
      'A user report showed the flow changed.',
    );
  ...
  it('non-main group cannot call Atlas', async () => {
    const requestId = 'atlas-other-request';
    ...
    expect(askAtlasSpy).not.toHaveBeenCalled();
  ...
  it('Reflect can call Atlas through its dedicated IPC namespace', async () => {
    const requestId = 'atlas-reflect-request';

    await processTaskIpc(
      {
        type: 'ask_atlas',
        requestId,
        instruction: 'Update the triage docs with a durable note',
      },
      REFLECT_GROUP_FOLDER,
      false,
      deps,
    );
    ...
    expect(askAtlasSpy).toHaveBeenCalledWith(
      REFLECT_GROUP_FOLDER,
      'Update the triage docs with a durable note',
      undefined,
    );
