...
export function getNewMessages(
  ...
  // Return the oldest unseen rows first so callers can page through large
  // backlogs without permanently skipping older messages.
  const sql = `
...
export function getMessagesSince(
  ...
  // Filter bot messages using both the is_bot_message flag AND the content
  // prefix as a backstop for messages written before the migration ran.
  // Return the oldest pending rows first so repeated calls can page forward.
  const sql = `
    SELECT id, chat_jid, sender, sender_name, content, timestamp, thread_ts, is_from_me
    FROM messages
...
export function getReflectMessagesSince(
  jids: string[],
  sinceTimestamp: string,
  limit: number = 200,
): NewMessage[] {
  ...
  const placeholders = jids.map(() => '?').join(',');
  const sql = `
    SELECT id, chat_jid, sender, sender_name, content, timestamp, thread_ts, is_from_me, is_bot_message
    FROM messages
    WHERE timestamp > ? AND chat_jid IN (${placeholders})
      AND content != '' AND content IS NOT NULL
    ORDER BY timestamp ASC, chat_jid ASC, id ASC
  ...
  return db.prepare(sql).all(sinceTimestamp, ...jids, limit) as NewMessage[];
}

export function getLatestMessageTimestampForChats(
  jids: string[],
): string | null {
  ...
  const row = db.prepare(sql).get(...jids) as { latest?: string | null };
  return row?.latest || null;
}

export function getThreadMessagesBefore(
  chatJid: string,
  threadTs: string,
  ...
  return db
    .prepare(sql)
    .all(chatJid, beforeTimestamp, threadTs, threadTs, limit) as NewMessage[];
}

export function getRecentTopLevelMessagesBefore(
  chatJid: string,
  beforeTimestamp: string,
  limit: number = 6,
  excludeId?: string,
