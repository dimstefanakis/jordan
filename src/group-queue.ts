...
export class GroupQueue {
  ...
  enqueueMessageCheck(
    ...
    if (state.active) {
      ...
      if (
        state.idleWaiting ||
        forcePreempt ||
        state.activeThreadTs !== options?.threadTs
      ) {
      ...
      logger.debug(
        {
          groupJid,
          forcePreempt,
          activeThreadTs: state.activeThreadTs,
          requestedThreadTs: options?.threadTs,
        },
  ...
  sendMessage(
    ...
    if (!state.active || !state.groupFolder || state.isTaskContainer)
      return false;
    if (state.activeThreadTs !== options?.threadTs) return false;
  ...
  private async runForGroup(
    ...
    try {
      ...
      state.activeThreadTs = undefined;
      this.activeCount--;
